...
```{r}
tidy_treat <- model_treat %>% tidy()
tidy_treatday <- model_treatday %>% tidy()

tribble(
	~Model, ~df, ~sumsq,
	"w/o Day (6 means)", tidy_treat$df[1], tidy_treat$sumsq[1],
	"With Day (10 means)", sum(tidy_treatday$df[1:2]), sum(tidy_treatday$sumsq[1:2])
) %>% 
	mutate(
		meansq = sumsq / df,
		fstat = c(meansq[1] / meansq[2], NA),
		pvalue = c(pf(fstat[1], df[1], df[2], lower.tail = FALSE), NA)
	) %>% 
	kbl(booktabs = TRUE, digits = c(0,0,1,1,3,3)) %>% 
	kable_styling(latex_options = "HOLD_position")
```

This gives no evidence that the means between days are different.
