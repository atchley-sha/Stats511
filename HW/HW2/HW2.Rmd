...
\]
which is equal to `r (psd <- sqrt( ((nf-1)*sdf+(nr-1)*sdr) / (nf+nr-2) ) %>% round(2))`, and the pooled standard error of the difference in means is $`r psd` * \sqrt{\frac{1}{`r nf`} + \frac{1}{`r nr`}}$,  which equals `r (pse <- (psd*sqrt(1/nf + 1/nr)) %>% round(2))`. There are $`r nf`+`r nr`- 2 = `r (df <- nf + nr - 2)`$ degrees of freedom associated with this estimate, and the 97.5th percentile of a _t_-distribution with `r df` degrees of freedom is `r qt(0.975, df) %>% round(2)`.
